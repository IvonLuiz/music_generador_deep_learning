...
print_usage() {
  cat <<'EOF'
Usage: train_transformer_prior_all_levels.sh [--levels LEVEL_LIST] [extra args...]
...
Any extra args are forwarded to:
  src/train_scripts/train_transformer_prior.py
EOF
}
...
echo "Starting sequential Jukebox Transformer prior training: ${LEVELS[*]}"
...
for level in "${LEVELS[@]}"; do
  ...
  echo "============================================="
  "$PYTHON_BIN" src/train_scripts/train_transformer_prior.py \
    --level "$level" \
    "${FORWARD_ARGS[@]}"
done

echo ""
echo "All requested Transformer priors trained successfully: ${LEVELS[*]}"
