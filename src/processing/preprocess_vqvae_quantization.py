...
from utils import load_config, load_vqvae_model
from windowed_data_utils import build_level_starts, build_timing_tensor, extract_window, source_stem
...
def _list_spectrogram_files(source_path: str) -> List[str]:
    return sorted(glob(os.path.join(source_path, '**', '*.npy'), recursive=True))
...
def _load_spectrogram(file_path: str) -> np.ndarray:
    spec = np.load(file_path)
    if spec.ndim == 3 and spec.shape[-1] == 1:
        spec = spec[:, :, 0]
    if spec.ndim != 2:
        raise ValueError(f'Expected spectrogram shaped (freq, time), got {spec.shape} for {file_path}')
...
def precompute_single_vqvae_indices(
    ...
    dataset_cfg['raw_path'] = source_path if source_mode == 'audio' else dataset_cfg.get('raw_path', source_path)
    dataset_cfg['processed_path'] = source_path if source_mode == 'spectrogram' else dataset_cfg.get('processed_path', source_path)

    if source_mode == 'audio':
        file_paths = _list_audio_files(source_path, audio_extensions)
    else:
        file_paths = _list_spectrogram_files(source_path)

    if max_files is not None:
        file_paths = file_paths[: int(max_files)]
    if not file_paths:
        raise FileNotFoundError(f'No source files found in {source_path} for source_mode={source_mode}')
    ...
    with open(manifest_path, 'w', encoding='utf-8') as manifest_file:
        ...
        for file_path in file_paths:
            ...
            spec = None
            if source_mode == 'spectrogram':
                spec = _load_spectrogram(file_path)
            ...
            for batch_start in range(0, len(starts), batch_size):
                ...
                if source_mode == 'audio':
                    ...
                    batch_specs = np.stack(
                        [extract_window(spec, start_frame=start, window_size=target_time_frames) for start in batch_starts],
                        axis=0,
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description='Precompute single VQ-VAE code indices for PixelCNN training.'
    )
    parser.add_argument('--vqvae_model', type=str, required=True, help='VQ-VAE run directory or checkpoint path.')
    parser.add_argument('--source_path', type=str, default=None, help='Raw audio or spectrogram directory. Defaults to the VQ-VAE config dataset path.')
    parser.add_argument('--output_dir', type=str, default='./data/processed/vqvae_pixelcnn_quantized/', help='Directory for quantized index .pt files.')
    parser.add_argument('--weights_file', type=str, default=None, help='Checkpoint filename when --vqvae_model is a directory.')
    parser.add_argument('--source_mode', type=str, choices=['audio', 'spectrogram'], default=None, help='Defaults to dataset.input_mode from the VQ-VAE config.')
    ...
    source_mode = args.source_mode or dataset_cfg.get('input_mode', 'spectrogram')
    source_mode = str(source_mode).strip().lower()
    if source_mode not in {'audio', 'spectrogram'}:
        raise ValueError("source_mode must be either 'audio' or 'spectrogram'")

    if args.source_path is None:
        args.source_path = dataset_cfg.get('raw_path') if source_mode == 'audio' else dataset_cfg.get('processed_path')
    if not args.source_path:
        raise ValueError('--source_path is required when it cannot be inferred from the VQ-VAE config.')
    ...
    print(f'Source mode/path: {source_mode} / {args.source_path}')
    ...
    precompute_single_vqvae_indices(
