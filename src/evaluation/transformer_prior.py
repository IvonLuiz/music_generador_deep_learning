...
@dataclass
class TransformerPriorSamplingConfig:
    """!
    @brief Settings for hierarchical Transformer prior sampling evaluation.
    """

    top_prior_path: str
    middle_prior_path: str
    bottom_prior_path: str
    bottom_vqvae_path: Optional[str] = None
...
@dataclass
class TransformerPriorBundle:
    """!
    @brief Loaded top/middle/bottom priors and inferred generation metadata.
    """

    top_prior: TransformerPriorConditioned
    middle_prior: TransformerPriorConditioned
    bottom_prior: TransformerPriorConditioned
    top_config: dict
    middle_config: dict
    bottom_config: dict
    top_seq_len: int
    middle_seq_len: int
    bottom_seq_len: int
    top_grid: Optional[list]
    middle_grid: Optional[list]
    bottom_grid: Optional[list]
    vqvae_cfg: dict
    bottom_condition_on_top: bool
    effective_vqvae_weights_file: str


@dataclass
class TransformerDecodeTarget:
    """!
    @brief Tokens and geometry needed to decode one generated transformer level.
    """
...
def _decode_tokens_to_audio(
    ...
    """!
    @brief Decode one level's generated tokens to spectrograms and audio files.
    @param level Jukebox level name: `top`, `middle`, or `bottom`.
    @param tokens Generated token timeline.
    @param grid Optional latent grid shape used for timeline decoding.
    @param vqvae_path Path to the level VQ-VAE run/config/checkpoint.
    @param weights_file Checkpoint filename to load from the VQ-VAE run.
    @param audio_config Structured spectrogram inversion settings.
    @param save_dir Output directory for decoded artifacts.
    @param device Torch device used for VQ-VAE decoding.
    @param chunk_time_cols Optional decode chunk width in latent time columns.
    @param decode_context_cols Context columns on each side during chunked decoding.
    @param trim_frames Optional final spectrogram frame count.
    """
    ...
    should_use_fixed_db = (
        audio_config.use_fixed_db_scale
        or (input_mode == 'audio' and audio_config.min_max_values_path is None)
    )
    ...
    if should_use_fixed_db:
        min_max_list = _fixed_db_min_max_values(
            decoded_specs.shape[0],
            audio_config.fixed_min_db,
            audio_config.fixed_max_db,
        )
        print(
            f'Using fixed dB denormalization for {level}: '
            f'normalized [0, 1] -> [{audio_config.fixed_min_db:.1f}, {audio_config.fixed_max_db:.1f}] dB'
        )
    else:
        min_max_values_path = _resolve_min_max_values_path(
            ...
            override_path=audio_config.min_max_values_path,
        )
...
def run_transformer_prior_sampling(
    top_prior_path: str,
    middle_prior_path: str,
    bottom_prior_path: str,
    ...
    """!
    @brief Sample from trained hierarchical Transformer priors and optionally decode audio.
    @param top_prior_path Path to top Transformer prior run/config/checkpoint.
    @param middle_prior_path Path to middle Transformer prior run/config/checkpoint.
    @param bottom_prior_path Path to bottom Transformer prior run/config/checkpoint.
    @param bottom_vqvae_path Optional VQ-VAE path override, mainly for bottom decoding.
    @param audio_method Legacy audio inversion method used when audio_config is omitted.
    @param num_samples Number of independent samples for non-full-length generation.
    @param temperature Sampling temperature.
    @param top_k Optional top-k sampling cutoff.
    @param weights_file Transformer prior checkpoint filename.
    @param timing Optional explicit timing tensor.
    @param full_length Whether to use full-length hierarchical generation.
    @param progress_interval Progress logging interval in generated tokens.
    @param decode_context_cols Context columns for timeline VQ-VAE decoding.
    @param min_max_values_path Legacy min/max metadata path used when audio_config is omitted.
    @param use_fixed_db_scale Legacy fixed dB flag used when audio_config is omitted.
    @param fixed_min_db Legacy fixed dB lower bound used when audio_config is omitted.
    @param fixed_max_db Legacy fixed dB upper bound used when audio_config is omitted.
    ...
    save_dir = os.path.join(os.path.abspath(os.path.expanduser(output_root)), current_time)
    ...
    if full_length:
        ...
        timing_total_frames = seconds_to_frames(timing_duration_seconds, sample_rate, hop_length)
        ...
        bottom_chunks_per_middle = (mid_tf // bot_tf) if mid_tf % bot_tf == 0 else None
        ...
        print('Full-length until: bottom')
        print(
    ...
    print('Generating complete top -> middle -> bottom hierarchy')
    if bottom_seq_len > 1024:
        print(
...
class TransformerPriorLoader:
    """!
    @brief Loads Transformer priors and exposes inferred metadata as a bundle.
    """

    @staticmethod
    def load_prior(model_layer: str, model_dir_or_file: str, device: torch.device, weights_file: str = 'best_model.pth'):
        """!
        @brief Load one top/middle/bottom Transformer prior.
        @param model_layer Prior level name.
        @param model_dir_or_file Run directory, config path, or checkpoint path.
        @param device Torch device.
        @param weights_file Checkpoint filename when a run directory is passed.
        @return Tuple `(prior, config, model_path)`.
        """
        return load_transformer_prior(model_layer, model_dir_or_file, device, weights_file)

    @staticmethod
    def load_bundle(config: TransformerPriorSamplingConfig, device: torch.device) -> TransformerPriorBundle:
        """!
        @brief Load all three priors and their shared generation metadata.
        @param config Sampling config.
        @param device Torch device.
        @return TransformerPriorBundle.
        """
        top_prior, top_config, _ = load_transformer_prior('top', config.top_prior_path, device, config.weights_file)
        middle_prior, middle_config, _ = load_transformer_prior('middle', config.middle_prior_path, device, config.weights_file)
        bottom_prior, bottom_config, _ = load_transformer_prior('bottom', config.bottom_prior_path, device, config.weights_file)
        bottom_prior_cfg = bottom_config.get('priors', {}).get('bottom_prior', {}) if isinstance(bottom_config, dict) else {}
        vqvae_cfg = bottom_config.get('vqvae', {}) if isinstance(bottom_config, dict) else {}
        return TransformerPriorBundle(
            top_prior=top_prior,
            middle_prior=middle_prior,
            bottom_prior=bottom_prior,
            ...
            or getattr(bottom_prior, 'second_conditioner', None) is not None,
            effective_vqvae_weights_file=vqvae_cfg.get('weights_file', 'best_model.pth'),
        )


class TransformerTokenSampler:
    """!
    @brief Shared token sampling helper for Transformer prior evaluation.
    """

    @staticmethod
    def generate_level_windows(*args, **kwargs):
        """!
        @brief Generate overlapped or independent token windows for one level.
        @return List of generated token arrays.
        """
        return _generate_level_windows(*args, **kwargs)


class TransformerTokenDecoder:
    """!
    @brief Decodes generated Transformer tokens through Jukebox VQ-VAE decoders.
    """

    @staticmethod
    def decode(
        level: str,
        target: TransformerDecodeTarget,
        vqvae_path: str,
        weights_file: str,
        audio_config: AudioInversionConfig,
        save_dir: str,
        ...
        """!
        @brief Decode one generated token target to spectrogram and audio artifacts.
        @param level Level name.
        @param target Token target and decode geometry.
        @param vqvae_path VQ-VAE path for this level.
        @param weights_file VQ-VAE checkpoint filename.
        @param audio_config Audio inversion config.
        @param save_dir Output directory.
        @param device Torch device.
        @param decode_context_cols Context columns for chunked decoding.
        """
        _decode_tokens_to_audio(
            level=level,
            ...
            save_dir=save_dir,
            device=device,
            chunk_time_cols=target.chunk_time_cols,
            decode_context_cols=decode_context_cols,
